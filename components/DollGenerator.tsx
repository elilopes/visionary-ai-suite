...
interface DollGeneratorProps {
    labels: any;
}

const DollGenerator: React.FC<DollGeneratorProps> = ({ labels }) => {
    ...
    const styles = [
        { name: "Barbie Style", prompt: "Convert this person into a plastic fashion doll (Barbie style). Smooth plastic skin, doll joints, fashion clothes." },
        { name: "Porcelain Doll", prompt: "Convert this person into a vintage porcelain doll (BJD style). Shiny ceramic texture, delicate features, victorian vibe." },
        { name: "Funko/Chibi", prompt: "Convert this person into a cute Chibi vinyl figure (Funko Pop style). Big head, small body, black eyes, plastic texture." },
        { name: "Rag Doll", prompt: "Convert this person into a handmade knitted rag doll. Yarn hair, button eyes, fabric texture, stitches." }
    ];
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            console.error("Error generating dolls:", e);
    ...
    const downloadCollage = () => {
        ...
        imgElements[0].onload = () => {
            ...
            link.download = 'doll-montage.png';
    ...
    return (
        <section className="bg-gray-800/50 p-6 rounded-lg shadow-lg border border-pink-500/30">
            <h3 className="text-2xl font-bold mb-2 text-pink-400">{labels.dollGeneratorTitle}</h3>
            <p className="text-gray-400 mb-6">{labels.dollGeneratorDescription}</p>
            ...
            <div className="space-y-4">
                ...
                >
                    ...
                    {isLoading ? labels.generatingPreview : labels.dollGeneratorButton}
                </button>
            </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {images.map((img, idx) => (
                            <div key={idx} className="bg-gray-900 rounded-lg p-2 border border-gray-700 flex flex-col items-center">
                                <h5 className="text-sm font-semibold text-pink-400 mb-2">{img.style}</h5>
                                ...
                                <a
                                    ...
                                    download={`doll-${img.style.replace(/\s+/g, '-').toLowerCase()}.png`}

...
interface WigTryOnProps {
    labels: any;
}

const WigTryOn: React.FC<WigTryOnProps> = ({ labels }) => {
    ...
    const styles = [
        { name: "Blonde Bob", prompt: "Change the person's hairstyle to a chic blonde bob cut. Keep the face exactly the same." },
        { name: "Long Wavy Black", prompt: "Change the person's hairstyle to long, wavy black hair. Keep the face exactly the same." },
        { name: "Red Curly", prompt: "Change the person's hairstyle to voluminous red curly hair. Keep the face exactly the same." },
        { name: "Pink Pixie", prompt: "Change the person's hairstyle to a stylish pink pixie cut. Keep the face exactly the same." }
    ];
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            console.error("Error generating wigs:", e);
    ...
    return (
        <section className="bg-gray-800/50 p-6 rounded-lg shadow-lg border border-purple-500/30">
            <h3 className="text-2xl font-bold mb-2 text-purple-400">{labels.wigTryOnTitle}</h3>
            <p className="text-gray-400 mb-6">{labels.wigTryOnDescription}</p>
            ...
            <div className="space-y-4">
                ...
                >
                    ...
                    {isLoading ? labels.generatingPreview : labels.wigTryOnButton}
                </button>
            </div>
                        ...
                        {images.map((img, idx) => (
                            <div key={idx} className="bg-gray-900 rounded-lg p-2 border border-gray-700 flex flex-col items-center">
                                ...
                                <a
                                    ...
                                    download={`wig-${img.style.replace(/\s+/g, '-').toLowerCase()}.png`}
